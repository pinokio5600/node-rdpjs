...
RdpClient.prototype.updateDesktopSize = function (width, height) {
	if (!this.connected) return;

    var event = pdu.data.desktopResizeEvent();
	event.obj.updateData.width.value = width;
	event.obj.updateData.height.value = height;
	event.obj.updateData.padding.value = 0;

    this.global.sendResizeEvent(event);
};
